...
def test_get_at_instant(tax_benefit_system) -> None:
...
def test_param_values(tax_benefit_system) -> None:
    dated_values = {
        "2015-01-01": 0.15,
        "2014-01-01": 0.14,
        "2013-01-01": 0.13,
...
def test_param_before_it_is_defined(tax_benefit_system) -> None:
    with pytest.raises(ParameterNotFound):
        _ = tax_benefit_system.get_parameters_at_instant(
            "1997-12-31"
        ).taxes.income_tax_rate


# The placeholder should have no effect on the parameter computation
def test_param_with_placeholder(tax_benefit_system) -> None:
    assert (
...
def test_stopped_parameter_before_end_value(tax_benefit_system) -> None:
    assert (
...
def test_stopped_parameter_after_end_value(tax_benefit_system) -> None:
    with pytest.raises(ParameterNotFound):
        _ = tax_benefit_system.get_parameters_at_instant(
            "2016-12-01",
        ).benefits.housing_allowance
...
def test_parameter_for_period(tax_benefit_system) -> None:
...
def test_wrong_value(tax_benefit_system) -> None:
    income_tax_rate = tax_benefit_system.parameters.taxes.income_tax_rate
    with pytest.raises(ValueError):
        income_tax_rate("test")


def test_parameter_repr(tax_benefit_system) -> None:
    parameters = tax_benefit_system.parameters
...
def test_parameters_metadata(tax_benefit_system) -> None:
...
def test_parameter_node_metadata(tax_benefit_system) -> None:
...
def test_parameter_documentation(tax_benefit_system) -> None:
...
def test_get_descendants(tax_benefit_system) -> None:
    all_parameters = {
