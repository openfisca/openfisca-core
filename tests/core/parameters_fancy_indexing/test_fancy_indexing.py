...
def test_double_fancy_indexing() -> None:
    zone = numpy.asarray(["z1", "z2", "z2", "z1"])
    housing_occupancy_status = numpy.asarray(["owner", "owner", "tenant", "tenant"])
    assert_near(P.single[housing_occupancy_status][zone], [100, 200, 400, 300])


def test_double_fancy_indexing_on_node() -> None:
...
def test_triple_fancy_indexing() -> None:
    family_status = numpy.asarray(
        [
            "single",
            "single",
            "single",
    ...
    housing_occupancy_status = numpy.asarray(
        ["owner", "owner", "tenant", "tenant", "owner", "owner", "tenant", "tenant"],
    )
...
def test_with_properties_starting_by_number() -> None:
...
def test_with_bareme() -> None:
    city_code = numpy.asarray(["75012", "75007", "75015"])
    with pytest.raises(NotImplementedError) as e:
        P_3[city_code]
    assert re.findall(
        r"'bareme.7501\d' is a 'MarginalRateTaxScale'",
        get_message(e.value),
    )
