...
# 371 - end, one function without date
...
def test_dates__no_attributes__one_formula__eternity():
    variable = tax_benefit_system.column_by_name['no_attributes__one_formula__eternity']
    assert variable is not None
...
# 371 - function, different start formats
...
add_variable_catch_assertion(tax_benefit_system, no_attribute__formulas__different_names__dates_overlap, "Dated formulas overlap")


def test_call__no_attribute__formulas__different_names__dates_overlap():
    month = '2005-05'
    simulation = new_simulation(tax_benefit_system, month)
    try:
        simulation.calculate('no_attribute__formulas__different_names__dates_overlap', month)
...
def test_dates__no_attribute__formulas__different_names__dates_overlap():
    # Check that AssertionError at variable adding prevents it from registration in the taxbenefitsystem.
    assert not hasattr(tax_benefit_system.column_by_name, "no_attribute__formulas__different_names__dates_overlap")
...
def test_dates__dated_attribute__one_formula__start():
    variable = tax_benefit_system.column_by_name['dated_attribute__one_formula__start']
    assert variable is not None
...
# 371 - end, function with dates intervals overlap.
...
def test_dates__dated_attribute_restrictive__one_formula():
    variable = tax_benefit_system.column_by_name['dated_attribute_restrictive__one_formula']
    assert variable is not None
    assert variable.end == '2001-01-01'
    ...
    assert formula['start_instant'] is not None
    assert formula['start_instant'].date == datetime.date(2001, 1, 1)


# 371 - end, functions different names (without dates overlap on functions)
...
def test_call__dated_attribute__formulas__different_names():
    month = '1979-12'
    simulation = new_simulation(tax_benefit_system, month)
    assert simulation.calculate('dated_attribute__formulas__different_names', month) == IntCol.default
...
def test_dates__dated_attribute__formulas__different_names():
    dated_function_nb = 2
    variable = tax_benefit_system.column_by_name['dated_attribute__formulas__different_names']
    assert variable is not None
    assert has_dated_attribute(variable)
...
# 371 - function start_instant > end

class dated_attribute__inactive_formulas(Variable):
    column = IntCol
    entity = Individu
    definition_period = MONTH
    label = u"Variable with restrictive dated attribute, more than 2 dated functions, all outside dated attributes interval."
    # start_date = datetime.date(1980, 1, 1)
    end = '1990-12-31'
...
tax_benefit_system.add_variable(dated_attribute__inactive_formulas)


def test_call__dated_attribute__inactive_formulas():
    month = '1985-01'
    simulation = new_simulation(tax_benefit_system, month)
    assert simulation.calculate('dated_attribute__inactive_formulas', month) == IntCol.default

    month = '2000-01'
    simulation = new_simulation(tax_benefit_system, month)
    assert simulation.calculate('dated_attribute__inactive_formulas', month) == IntCol.default  # Raised an error before DatedVariable update.
...
def test_dates__dated_attribute__inactive_formulas():
    dated_function_nb = 4

    variable = tax_benefit_system.column_by_name['dated_attribute__inactive_formulas']
    assert variable is not None
    assert has_dated_attribute(variable)
    ...
    formula = variable.formula_class.dated_formulas_class[i]
    assert formula is not None
    # Raised an error before DatedVariable update.
    assert formula['start_instant'] is not None
    assert formula['start_instant'].date == datetime.date(2000, 1, 1)
...
# 371 - end, all functions start_instant < end

class dated_attribute__active_formulas(Variable):
    column = IntCol
    entity = Individu
    definition_period = MONTH
    label = u"Variable with end attribute, more than 2 dated functions, all inside dates interval."
...
tax_benefit_system.add_variable(dated_attribute__active_formulas)


def test_call__dated_attribute__active_formulas():
    month = '1999-12'
    simulation = new_simulation(tax_benefit_system, month)
    assert simulation.calculate('dated_attribute__active_formulas', month) == IntCol.default

    month = '2000-01'
    simulation = new_simulation(tax_benefit_system, month)
    assert simulation.calculate('dated_attribute__active_formulas', month) == 100

    month = '2006-01'
    simulation = new_simulation(tax_benefit_system, month)
    assert simulation.calculate('dated_attribute__active_formulas', month) == 200

    month = '2011-01'
    simulation = new_simulation(tax_benefit_system, month)
    assert simulation.calculate('dated_attribute__active_formulas', month) == 300

    month = '2016-01'
    simulation = new_simulation(tax_benefit_system, month)
    assert simulation.calculate('dated_attribute__active_formulas', month) == 400


def test_dates__dated_attribute__active_formulas():
    dated_function_nb = 4

    variable = tax_benefit_system.column_by_name['dated_attribute__active_formulas']
    assert variable is not None
    assert has_dated_attribute(variable)
