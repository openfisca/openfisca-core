...
def test_permanent_variable_filled(single):
    ...
    holder.set_input(periods.build(periods.ETERNITY), value)
...
def test_delete_arrays(single):
    ...
    salary_holder.set_input(periods.build(2017), numpy.asarray([30000]))
    salary_holder.set_input(periods.build(2018), numpy.asarray([60000]))
    ...
    salary_holder.set_input(periods.build(2018), numpy.asarray([15000]))
    assert simulation.person('salary', '2017-01') == 2500
...
def test_set_input_dispatch_by_period(single):
    ...
    holders.set_input_dispatch_by_period(holder, periods.build(2019), 'owner')
