...
period = periods.Period.build('2017-12')
...
def test_permanent_variable_filled(single):
    ...
    holder.set_input(periods.Period.build(periods.ETERNITY), value)
...
def test_delete_arrays(single):
    ...
    salary_holder.set_input(periods.Period.build(2017), numpy.asarray([30000]))
    salary_holder.set_input(periods.Period.build(2018), numpy.asarray([60000]))
    ...
    salary_holder.set_input(periods.Period.build(2018), numpy.asarray([15000]))
    assert simulation.person('salary', '2017-01') == 2500
...
def test_set_input_dispatch_by_period(single):
    ...
    holders.set_input_dispatch_by_period(holder, periods.Period.build(2019), 'owner')
...
def test_cache_disk(couple):
    simulation = couple
    simulation.memory_config = force_storage_on_disk
    month = periods.Period.build('2017-01')
...
def test_known_periods(couple):
    ...
    simulation.memory_config = force_storage_on_disk
    month = periods.Period.build('2017-01')
    month_2 = periods.Period.build('2017-02')
    holder = simulation.person.get_holder('disposable_income')
    data = numpy.asarray([2000, 3000])
    holder.put_in_cache(data, month)
