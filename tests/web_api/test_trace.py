...
def test_trace_with_repetition():
    ...
    assert_equal(response.status_code, OK)
    response_json = json.loads(response.data)
    basic_income_dep_1 = dpath.util.get(response_json, 'trace/total_benefits<2017-01>/dependencies/basic_income<2017-01>/dependencies')
    basic_income_dep_2 = dpath.util.get(response_json, 'trace/disposable_income<2017-01>/dependencies/basic_income<2017-01>/dependencies')
