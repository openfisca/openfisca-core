...
def test_trace_basic(test_client):
    ...
    response = test_client.post(
        "/trace", data=simulation_json, content_type="application/json"
    )
    assert response.status_code == client.OK
    response_json = json.loads(response.data.decode("utf-8"))
    disposable_income_value = dpath.util.get(
        response_json, "trace/disposable_income<2017-01>/value"
    )
    ...
    disposable_income_dep = dpath.util.get(
        response_json, "trace/disposable_income<2017-01>/dependencies"
    )
    assert_items_equal(
    ...
    basic_income_dep = dpath.util.get(
        response_json, "trace/basic_income<2017-01>/dependencies"
    )
...
def test_trace_enums(test_client):
    ...
    housing_status = dpath.util.get(
        response_json, "trace/housing_occupancy_status<2017-01>/value"
    )
