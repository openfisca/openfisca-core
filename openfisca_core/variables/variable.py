...
class Variable:
    ...
    def set(
        ...
        if required and value is None:
            raise ValueError(
                f"Missing attribute '{attribute_name}' in definition of variable '{self.name}'."
            )
    ...
    def set_end(self, end):
        if end:
            try:
                ...
                raise ValueError(
                    f"Incorrect 'end' attribute format in '{self.name}'. 'YYYY-MM-DD' expected where YYYY, MM and DD are year, month and day. Found: {end}"
                )
