...
class TaxBenefitSystem:
    ...
    def apply_reform(self, reform_path: str) -> TaxBenefitSystem:
        ...
        try:
            ...
            raise ValueError(
                f"`{reform_path}` does not seem to be a path pointing to a reform. A path looks like `some_country_package.reforms.some_reform.`"
            )
        try:
        ...
        if not issubclass(reform, Reform):
            raise ValueError(
                f"`{reform_path}` does not seem to be a valid Openfisca reform."
            )
