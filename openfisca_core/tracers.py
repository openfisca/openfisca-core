...
class TracingParameterNodeAtInstant:
    ...
    def get_traced_child(self, child, key):
        ...
        if not isinstance(key, str) or isinstance(
            self.parameter_node_at_instant, VectorialParameterNodeAtInstant
        ):
...
class TraceNode:
    ...
    def formula_time(self):
        result = self.calculation_time(round_=False) - sum(
...
class FlatTrace:
    ...
    def get_trace(self):
        ...
        for node in self._full_tracer.browse_trace():
            trace.update(
                {  # We don't want cache read to overwrite data about the initial calculation. We therefore use a non-overwriting update.
                    key: node_trace
...
class ComputationLog:
    ...
    def _get_node_log(self, node, depth, aggregate) -> List[str]:
        def print_line(depth, node) -> str:
            ...
            return "{}{}<{}> >> {}".format(
                "  " * depth, node.name, node.period, formatted_value
            )
        ...
        children_logs = self._flatten(
            self._get_node_log(child, depth + 1, aggregate) for child in node.children
        )
    ...
    def aggregate_calculation_times(self, flat_trace: Dict) -> Dict[str, Dict]:
        def _aggregate_calculations(calculations):
            ...
            return {
                ...
                "avg_calculation_time": TraceNode.round(
        ...
        return {
            ...
            for variable_name, calculations in groupby(
                all_calculations, lambda calculation: calculation[0].split("<")[0]
            )
        }
