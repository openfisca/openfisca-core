...
class dom_tom(Variable):
    ...
    def formula(self, simulation, period):
        ...
        return numpy.logical_or(
            startswith(city_code, "97"), startswith(city_code, "98")
        )
...
def check_revenu_disponible(year, city_code, expected_revenu_disponible):
    simulation = simulations.Simulation(
        period=periods.period(year), tax_benefit_system=tax_benefit_system
    )
    ...
    simulation.get_or_new_holder("city_code").array = numpy.array(
        [city_code, city_code, city_code]
    )
    famille.members_entity_id = numpy.array([0, 0, 1, 1, 2, 2])
    simulation.get_or_new_holder("salaire_brut").array = numpy.array(
        [0.0, 0.0, 50000.0, 0.0, 100000.0, 0.0]
    )
    revenu_disponible = simulation.calculate("revenu_disponible")
    assert_near(
        revenu_disponible, expected_revenu_disponible, absolute_error_margin=0.005
    )
...
def main():
    ...
    check_revenu_disponible(
        2010, "75101", numpy.array([1200, 1200, 25200, 1200, 50400, 1200])
    )
    check_revenu_disponible(
        2011, "75101", numpy.array([2400, 2400, 25200, 2400, 50400, 2400])
    )
    check_revenu_disponible(
        2012, "75101", numpy.array([2400, 2400, 25200, 2400, 50400, 2400])
    )
    check_revenu_disponible(
        2013, "75101", numpy.array([3600, 3600, 25200, 3600, 50400, 3600])
    )

    check_revenu_disponible(
        2009, "97123", numpy.array([-70.0, -70.0, 25130.0, -70.0, 50330.0, -70.0])
    )
    check_revenu_disponible(
        2010, "97123", numpy.array([1130.0, 1130.0, 25130.0, 1130.0, 50330.0, 1130.0])
    )
    check_revenu_disponible(
        2011, "98456", numpy.array([2330.0, 2330.0, 25130.0, 2330.0, 50330.0, 2330.0])
    )
    check_revenu_disponible(
        2012, "98456", numpy.array([2330.0, 2330.0, 25130.0, 2330.0, 50330.0, 2330.0])
    )
    check_revenu_disponible(
        2013, "98456", numpy.array([3530.0, 3530.0, 25130.0, 3530.0, 50330.0, 3530.0])
    )
