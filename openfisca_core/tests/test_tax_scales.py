...
def test_simple_linear_average_rate_tax_scale():
    ...
    assert_near(marginal_tax_scale.calc(base_node).value, [0, .05, .1, .2, .3, .3], absolute_error_margin = 1e-10)
...
def test_linear_average_rate_tax_scale():
    ...
    marginal_tax_scale.add_bracket(1, 0.1)
    marginal_tax_scale.add_bracket(2, 0.2)
    assert_near(marginal_tax_scale.calc(base_node).value, [0, .05, .1, .2], absolute_error_margin = 0)
    ...
    assert_near(average_tax_scale.calc(base_node).value, [0, 0.0375, 0.1, 0.125], absolute_error_margin = 1e-10)
...
def test_round_marginal_tax_scale():
    ...
    assert_near(
        marginal_tax_scale.calc(base_node).value,
        [10, 10.02, 10.0002, 10.06, 10.0006, 10.05, 10.0005],
        absolute_error_margin = 1e-10,
        )
    assert_near(
        marginal_tax_scale.calc(base_node, round_base_decimals = 1).value,
        [10, 10., 10., 10.1, 10., 10, 10.],
        absolute_error_margin = 1e-10,
        )
    assert_near(
        marginal_tax_scale.calc(base_node, round_base_decimals = 2).value,
        [10, 10.02, 10., 10.06, 10.00, 10.05, 10],
        absolute_error_margin = 1e-10,
        )
    assert_near(
        marginal_tax_scale.calc(base_node, round_base_decimals = 3).value,
