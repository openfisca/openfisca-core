...
class ParameterAtInstant(object):
    ...
    def validate(self, data):
        ...
        if not isinstance(value, ALLOWED_PARAM_TYPES):
            raise ParameterParsingError(
                "Invalid value in {} : {}".format(self.name, value), self.file_path
            )
    ...
    def build_from_node(node):
        ...
        recarray = np.array(
            ...
            dtype=[
                (
                    subnode_name,
                    subnode.dtype if isinstance(subnode, np.recarray) else "float",
                )
        ...
        return VectorialParameterNodeAtInstant(
            node._name, recarray.view(np.recarray), node._instant_str
        )
        ...
        # If the key is a vector, e.g. ['zone_1', 'zone_2', 'zone_1']
            ...
            if not np.issubdtype(key.dtype, np.str_):
                ...
                if key.dtype == object and issubclass(type(key[0]), Enum):
                    enum = type(key[0])
                    key = np.select(
                        [key == item for item in enum], [item.name for item in enum]
                    )
            ...
            default = np.full_like(
                self.vector[key[0]], np.nan
            ...
            if np.issubdtype(result.dtype, np.record):
                return VectorialParameterNodeAtInstant(
                    self._name, result.view(np.recarray), self._instant_str
                )
...
def _parse_child(child_name, child, child_path):
    if "values" in child:
        ...
        [INSTANT_PATTERN.match(str(key)) for key in child.keys()]
    ):
        return Parameter(child_name, child, child_path)
    else:
