...
class Famille(object):
    ...
    def BaseRessourcePF(self, P):
        ...
        La Caf tient compte de vos revenus, et des revenus de la personne avec laquelle 
        vous vivez en couple.
        Elle retient :
          - tous les revenus imposables perçus en France
          - les indemnités journalières maladie, maternité, accident du travail, maladie 
            professionnelle
          - les revenus perçus hors de France, et aussi, ceux versés par une organisation 
            internationale
        Elle effectue certaines déductions fiscales habituelles (10% sur les salaires et 
        les pensions, etc.).
        Selon votre situation (chômage, admission à la retraite, etc.), elle fait des 
        abattements ou neutralise certaines catégories de revenus.
        A partir du 1er juillet 2007, votre Caf, pour le calcul de votre Aah, continue à 
        prendre en compte les ressources de votre foyer diminuées de 20%.
        Notez, dans certaines situations, la Caf évalue forfaitairement vos ressources à 
        partir de votre revenu mensuel.        
        
        
        Code de la sécurité sociale - Article R532-3
        
        Les ressources retenues sont celles perçues pendant l'année civile de référence.
        L'année civile de référence est l'avant-dernière année précédant la période de paiement.
        Sous réserve des dispositions des articles R. 532-4 à R. 532-8 et des alinéas suivants 
        du présent article, les ressources prises en considération s'entendent du total des 
        revenus nets catégoriels retenus pour l'établissement de l'impôt sur le revenu d'après 
        le barème des revenus taxés à un taux proportionnel ou soumis à un prélèvement libératoire 
        de l'impôt sur le revenu, ainsi que les revenus perçus hors de France ou versés par une 
        organisation internationale, à l'exclusion des revenus des enfants ayant fait l'objet 
        d'une imposition commune et après :

        a) La déduction au titre des créances alimentaires mentionnée au 2° du II de l'article 
        156 du code général des impôts et majorées dans les conditions prévues au 7 de l'article 
        ...
        Sont exclus du décompte des ressources les arrérages des rentes viagères constituées en 
        faveur d'une personne handicapée et mentionnées à l'article 199 septies (2°) du code général 
        des impôts.

        Il est fait abstraction des déductions opérées en vertu de l'article 156-I du code général 
        des impôts au titre des reports des déficits constatés au cours d'une année antérieure à celle 
        ...
        ne proviennent pas d'une activité salariée et que ces ressources ne sont pas connues au moment 
        de la demande ou du réexamen des droits, il est tenu compte des dernières ressources connues et 
        déterminées dans les conditions prévues aux alinéas précédents. Ces ressources sont revalorisées 
        par application du taux d'évolution en moyenne annuelle de l'indice général des prix à la 
        consommation des ménages pour l'année civile de référence figurant dans le rapport économique et 
        financier annexé au projet de loi de finances.

        En cas de concubinage, il est tenu compte du total des ressources perçues par chacun des concubins 
        durant l'année de référence ; ces ressources sont déterminées dans les conditions prévues aux 
        alinéas précédents.
        '''
        self.revChef  = self.tsprC + self.hsupC + self.rpnsC
        self.revPart  = self.tsprP + self.hsupP + self.rpnsP
        del self.tsprC, self.rpnsC, self.tsprP, self.rpnsP
...
#          
    ...
    def AF(self,Param):
        ...
        af_nbenf_maj1 = self.NbEnfA(MajAge.age1, MajAge.age2-1, P)
        af_nbenf_maj2 = self.NbEnfA(MajAge.age2, P.af.age2, P)
        ...
        self.af_base = af_base.sum(axis=0)
        self.af_majo = af_majo.sum(axis=0)
        self.af_ai20 = af_ai20.sum(axis=0)
        
        self.af_brut = self.af_base + self.af_majo + self.af_ai20
        self.af_tot_m = round((af_base + af_majo + af_ai20)*(1-crds),2)
    ...
    def ASF(self,Param):
        ...
        asf_nbenf = self.NbEnf(age1,age2)
    ...
    def ARS(self,Param):
        ...
        enf_05    = self.NbEnf(P.ars.agep-1,P.ars.agep-1)[8,:]  # 6 ans avant le 31 janvier
        ...
        enf_primaire = enf_05 + self.NbEnf(P.ars.agep,P.ars.agec-1)[8,:]
        enf_college = self.NbEnf(P.ars.agec,P.ars.agel-1)[8,:]
        enf_lycee = self.NbEnf(P.ars.agel,P.ars.ages)[8,:]
        
        arsnbenf =   enf_primaire + enf_college + enf_lycee
        
        ars_plaf_res = P.ars.plaf*(1+ arsnbenf*P.ars.plaf_enf_supp)
        arsbase  = P.af.bmaf*(P.ars.tx0610*enf_primaire + \
    ...
    def pajeNais(self,Param):
        ...
        # TODO à mensualiser    
        benjamin = self.age_en_mois_benjamin(P,2)
        prime =  (benjamin==-2)
        nbnais    = sum(self.NbEnf(-1,-1)*prime,axis=0)

        # Et on compte le nombre d'enfants AF présents  pour le seul mois de la prime
        nbaf = sum(self.NbEnf(P.af.age1,P.af.age2)*prime,axis=0)
        ...
        self.nais_brut= nais_brut  
        

    @famille_wrapper()            
    def pajeClca(self, Param):
        '''
        ...
        benjamin = self.age_en_mois_benjamin(P)
       
        condition1 =(self.NbEnf(P.af.age1,P.af.age2)==1)*(benjamin>=0)*(benjamin<6)
        age = np.floor(benjamin/12)
        condition2 = ( age <= (P.paje.base.age-1))            
        condition = (self.NbEnf(0,P.af.age2)>=2)*condition2 + condition1 
...
#        du travail fixée dans l'entreprise), le montant des plafonds Cmg est divisé par 2.
        ...
        clmg_brut = P.af.bmaf*( (self.NbEnf(0,P.paje.clmg.age1-1)>0) + 0.5*(self.NbEnf(P.paje.clmg.age1,P.paje.clmg.age2-1)>0) )*(
            empl_dir*(
                (self.BRpftot < seuil1)*P.paje.clmg.empl_dir1 +
                ( (self.BRpftot >= seuil1) & (self.BRpftot < seuil2) )*P.paje.clmg.empl_dir2 +
                (self.BRpftot >= seuil2)*P.paje.clmg.empl_dir3) +
            ass_mat*(
                (self.BRpftot < seuil1)*P.paje.clmg.ass_mat1 +
                ( (self.BRpftot >= seuil1) & (self.BRpftot < seuil2) )*P.paje.clmg.ass_mat2 +
                (self.BRpftot >= seuil2)*P.paje.clmg.ass_mat3)  +
            gar_dom*(
                (self.BRpftot < seuil1)*P.paje.clmg.domi1 +
                ( (self.BRpftot >= seuil1) & (self.BRpftot < seuil2) )*P.paje.clmg.domi2 +
                (self.BRpftot >= seuil2)*P.paje.clmg.domi3))        
...
#        vous ne pouvez pas bénéficier du Cmg.         
    ...
    def pajeColca(self,Param):    
        ...
        P = Param.fam
        condition = (self.age_en_mois_benjamin(P,0) < 12*P.paje.colca.age )*(self.age_en_mois_benjamin(P,0) >=0)   
        nbenf = self.NbEnf(P.af.age1,P.af.age2)
...
#Vos enfants doivent être nés avant le 1er janvier 2004.
        ...
        afeama_brut = self.NbEnf(P.af.age1,P.afeama.age-1)*P.af.bmaf*( 
                ...
                ( (self.BRpftot >= seuil1) & (self.BRpftot < seuil2) )*P.afeama.taux_median +
                (self.BRpftot >= seuil2)*P.afeama.taux_maxi)
    ...
    def AlFormule(self,P):
        ...
        # self.isol : ménage isolé
        # self.coup: ménage en coup (rq : self.coup = ~self.isol.
...
#        TODO avoir le % d'incapacité ?        
        ...
        if hasattr(self, "asiC_m") & hasattr(self, "aspaC_m"): 
            self.BRaah = self.BRpftot/12 + self.asiC_m + self.aspaC_m + self.asiP_m + self.aspaP_m
            del self.asiP_m, self.asiC_m, self.aspaP_m, self.aspaC_m
        elif hasattr(self, "mv_m"):     
            self.BRaah = self.BRpftot/12 + self.mv_m
            del self.mv_m
