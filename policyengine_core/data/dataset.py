...
class Dataset:
    ...
    label: str = None
    options: dict = None
    country_package_name: str = None # Used to decide where to store the microdata files.
    ...
    def get_file_path(self, options: dict = None) -> Path:
        ...
        try:
            ...
            country_package = importlib.import_module(self.country_package_name)
            country_dir = country_package.COUNTRY_DIR
        except:
            raise ValueError(f"Could not import the country package {self.country_package_name}.")
    ...
    def get_saved_datasets(self) -> List[dict]:
        ...
        for file in folder.glob(f"{self.name}*.h5"):
            options = {}
            options_string = re.search(rf"{self.name}_(.*)\.h5", file.name).group(1)
