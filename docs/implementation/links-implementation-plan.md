# Plan d'implémentation du système de liens
...
### Ce qu'on veut en PLUS pour tester les liens

| Lien | Type | Exemple d'usage | Pourquoi |
|---|---|---|---|
| `person.mother` | Many2One (Person→Person) | `person.mother("salary", period)` | Tester les liens intra-entité |
| `person.father` | Many2One (Person→Person) | `person.father("age", period)` | Tester un 2ème lien intra-entité |
| `person.spouse` | Many2One (Person→Person) | `person.spouse("salary", period)` | Tester un lien symétrique |
| `person.children` | One2Many (Person→Person via `mother`/`father`) | `person.children.count()` | Tester le lien inverse |
| `person.mother.household` | Chaîné | `person.mother.household("rent", period)` | Tester la composition de liens |
...
1. **Vérifier les tests existants**
   ```bash
   cd /home/benjello/projets/openfisca-core
...
3. **Vérifier que country-template tourne contre core local**
   ```bash
   cd /home/benjello/projets/country-template
...
**aucun test existant ne casse** (les Link ne sont pas encore branchés).
...
des liens intra-entité (mère, père, conjoint).
...
#### 2. Enregistrer les liens : `entities.py` (modifié)
...
# ← NOUVEAU : déclarer les liens intra-entité
...
#### 3. Variables utilisant les liens : `variables/family_benefits.py`
...
## Phase 3 : Brancher les liens sur Population (2-3 jours)
...
class CorePopulation:
    def __getattr__(self, name):
        # Vérifier si c'est un lien déclaré
...
class LinkAccessor:
    ...
    def __getattr__(self, name):
        ...
        # C'est un projector existant (person.mother.household)
        # → il faut d'abord résoudre le lien, puis projeter
...
# Y compris les nouveaux tests de liens
pytest openfisca_country_template/tests/family_links.yaml -v
```

---

## Phase 4 : Auto-génération des liens depuis GroupEntity (1-2 jours)

**Objectif** : Les liens `person.household` (actuellement via les
...
class TaxBenefitSystem:
    ...
    def _auto_generate_links(self):
        for entity in self.group_entities:
            # Sur Person : lien Many2One vers le groupe
            ...
            # Sur le groupe : lien One2Many vers Person
            entity.add_link(
                One2ManyLink(
                    name="members",
                    field=f"{entity.key}_id",
...
variables utilisant la syntaxe lien fonctionnent aussi :
...
class EntityToPersonProjector(Projector):
    def transform(self, result):
        ...
        # APRÈS : utilise le lien sous-jacent
...
1. **Tous les tests existants passent** (non-régression absolue)
2. Les projectors utilisent les liens en interne
3. L'API externe est identique

---

## Phase 6 : Tests d'intégration complets (1-2 jours)
...
- Les projectors font un **fallback** sur l'ancien code si le lien
  n'est pas trouvé
...
1. D'abord chercher dans les attributs normaux
2. Puis dans les liens déclarés
3. Puis dans les projectors existants
4. Sinon `AttributeError`

Le risque est de casser l'accès à des attributs qui fonctionnaient
avec les projectors. Il faut vérifier que le mécanisme existant
...
est une Variable normale (int). Aucun changement nécessaire.
...
Après les liens familiaux de base, on pourra enrichir avec :
- `employer_id` → lien person.employer (prépare les simulations firmes)
- formules plus complexes utilisant les liens chaînés
