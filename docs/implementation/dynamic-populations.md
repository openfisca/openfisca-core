...
6. [Impact sur les projets existants](#6-impact-sur-les-projets-existants)
...
|---|---|---|
| `count` | `_core_population.py:35` | Attribut entier simple, assigné une seule fois |
...
**Transposition OpenFisca** : Ajouter cet attribut dans `CorePopulation`. Le `count` actuel devient dérivé : `count = np.sum(id_to_rownum != -1)`.
...
**Transposition OpenFisca** : Il faudra un **mode séquentiel** (optionnel) où la simulation avance période par période, permettant des mutations entre les périodes.
...
class CorePopulation:
    # === Attributs existants (inchangés en mode statique) ===
    entity: CoreEntity
    simulation: Simulation
    _holders: dict[str, Holder]

    # === Nouveaux attributs pour le mode dynamique ===
    ...
    def count(self) -> int:
        ...
        return self._count  # mode statique : attribut fixe comme avant
...
**Objectif** : Refactorer les attributs `count` et `ids` en propriétés sans changer le comportement.
...
**Risque** : Faible. Les propriétés sont transparentes pour le code existant.
...
Ajouter les attributs et méthodes suivants :
...
**Note** : En mode dynamique, la vérification de taille est relaxée car les arrays de périodes différentes peuvent avoir des tailles différentes. La cohérence est garantie par le `_period_index`.
...
class Holder:
    def _extend_arrays(self, count: int, values: ndarray | None = None) -> None:
        ...
        nécessite le period_index correspondant).
        """
...
class GroupPopulation(Population):
    ...
    def snapshot_period(self, period: Period) -> None:
        """Sauvegarder l'état complet pour la période."""
...
def run_period(self, period: Period, processes: list[callable] | None = None) -> None:
    ...
    Args:
        ...
        processes: Liste optionnelle de fonctions (mutations) à exécuter.
...
**Objectif** : Permettre aux formules d'accéder correctement aux données de périodes passées malgré les changements de population.
...
### 5.2 Formules qui accèdent au passé
...
- Compression : stocker les deltas plutôt que les tableaux complets.
- Disk-backed : utiliser le `OnDiskStorage` existant pour les anciennes périodes.

### 5.6 Compatibilité avec les projets existants

**Problème** : Les pays (openfisca-france, etc.) ne doivent pas être affectés.

**Solution** : Le mode `dynamic=False` est le défaut. Aucun changement de comportement. Les projets existants fonctionnent exactement comme avant. Le mode dynamique est opt-in.
...
| `definition_period` | Exemples | Conversions automatiques |
|---|---|---|
| `ETERNITY` | date de naissance, sexe | Jamais recalculé |
| `YEAR` | impôt sur le revenu, RSA éligibilité | Somme des mois, etc. |
...
class impot_revenu(Variable):
    ...
    def formula(person, period):
        # Somme automatique des salaires mensuels
...
# Mariage/divorce → événementiel (n'importe quand)
...
for year in range(2020, 2050):
    ...
    # Calculs annuels (agrègent les mois automatiquement)
...
La recommandation est de **snapshoter au rythme des mutations** :
- Si les mutations sont annuelles → snapshot annuel
- Si les mutations sont mensuelles → snapshot mensuel
- La fenêtre glissante (opt. 1) limite l'impact mémoire
...
| Complexité | Simple | Plus complexe (ordre des calculs) |
| Précision | 🟡 | ✅ Meilleure |
...
### 5.9 Déterminisme : l'invariant fondamental
...
simulation.evolve_population("2024-05")  # mutation à mai ← RÉTROACTIF !
  # → change composition("2024-05") → invalide le résultat pour juin
```
...
dépendances. Mais on peut utiliser une approche **conservatrice et triviale** :

> En mode dynamique, le temps avance dans un seul sens. Une fois qu'on a calculé
...
class PopulationStateTracker:
    ...
    (coûteux), on adopte une approche conservatrice : TOUT calcul à la
...
- ✅ **O(1)** — pas d'analyse de graphe, juste une comparaison d'entiers
...
C'est le même problème que les **conflits lecture/écriture** en transactions :
...
#### Contrainte sur les formules d'évolution

Si la composition est modélisée comme une variable temporelle (cf. analyse
des liens), sa formule ne doit dépendre que de variables à des **périodes
...
class Simulation:
    ...
    def calculate(self, variable_name, period, ...):
        # Hook AVANT le calcul existant — O(1)
        if self._dynamic:
...
## 6. Impact sur les projets existants
...
| Nouveaux attributs `_dynamic`, etc. | 🟢 Nul (opt-in) |
...
- Combinaison OpenFisca + LIAM2 dans un seul framework
...
## Annexe A : Correspondance OpenFisca ↔ LIAM2
...
|---|---|---|
| `populations/_core_population.py` | 0, 1, 2 | Propriétés count/ids, attributs dynamiques, add/remove |
...
**Recommandation pour le PoC** : Se concentrer sur les Phases 0, 1, 2 et un test de bout en bout simplifié (sans les entités de groupe). Cela représente environ **7-10 jours** et permettra de valider l'approche avant d'investir dans les phases suivantes.
...
La plupart des formules ne regardent que quelques périodes en arrière
...
class Simulation:
    ...
    def gc_period(self, period, keep=None):
        """Supprimer toutes les variables d'une période, sauf celles à garder.
...
for year in range(2020, 2050):
    ...
    # pour les formules futures (celles qui regardent le passé)
...
Utiliser le `OnDiskStorage` existant d'OpenFisca pour les périodes
...
| Ressource | Sans optimisation | Avec optimisations | Réduction |
|---|---|---|---|
...
ou le **plan d'exécution** d'un moteur de base de données.
...
déclarées et calcule la **durée de vie** de chaque variable :

```python
class SimulationPlanner:
    """Analyse statique d'un exercice de simulation."""
    ...
    def analyze(self, plan: SimulationPlan,
        ...
        # 2. Calculer la durée de vie de chaque variable
...
|---|---|---|
| **Mémoire pic** | Somme des variables vivantes au pire moment | Savoir si ça tient en RAM |
| **Durée de vie** par variable | Écart max entre period et dernier lecteur | Savoir quand évacuer |
| **Variables intermédiaires** | Celles qui ne sont lues que 1 fois | Évacuation immédiate |
| **Variables longue durée** | Celles lues avec lookback (ex: `ADD` sur 12 mois) | Garder K périodes |
...
### D.8 Recommandations révisées avec le planificateur
...
mémoire contraint. Il transforme le GC manuel (opt. 3) en un GC **automatique
et optimal**.
...
- **Limite** : Pas de compression, pas de partage mémoire, pas d'éviction
...
- **Technologie** : fichiers `.npy` individuels (un par variable × période)
...
- **Limite** : Pas de compression, append coûteux (copie O(N))
...
### E.3 Comparaison OpenFisca vs LIAM2
...
**Verdict** : ✅ **Meilleur candidat** pour remplacer `.npy` et HDF5.
...
**Verdict** : ✅ **Très bon candidat** — la structure hiérarchique
...
| Inconvénient | Détail |
|---|---|
| Complexe | Gestion manuelle du cycle de vie |
...
### E.6 Recommandation : stratégie par couche
...
Zarr est le remplacement naturel d'`OnDiskStorage` car sa structure
...
Pendant la simulation, le moteur accède aux données **une variable à la fois** :
...
**En résumé** : Zarr **à l'intérieur** (il remplace `OnDiskStorage`),
...
OpenFisca est **déjà largement vectorisé** grâce à numpy. Les formules
opèrent sur des arrays complets de tous les individus simultanément :
...
**Ce que c'est** : Compilateur JIT qui transforme les fonctions Python
...
**Verdict** : ✅ **Meilleur candidat** pour les hotspots 1-3.
...
| Avantage | Détail |
|---|---|
| Transparent | Code Python existant |
...
On pourrait l'utiliser plus largement dans les formules :
...
#### Bottleneck — fonctions glissantes optimisées
...
| Formules indépendantes | Multiprocessing | 10 s | 3 s | **3×** |

### F.5 Recommandation par priorité
...
| **3. 🟡 Moyenne** | Utiliser `numexpr` dans plus de formules | 2 jours | Déjà en dépendance | 3× |
| **4. 🟡 Moyenne** | Ajouter Numba pour `members_position` + `get_rank` | 1 jour | numba | 1000× |
| **5. 🟢 Basse** | Paralléliser les formules indépendantes | 5 jours | Aucune | 3× |
...
Les formules elles-mêmes (le code écrit par les modélisateurs dans les
...
sur les 3 fonctions de groupement listées ci-dessus. L'effort
...
**Verdict** : 🔴 **Mauvais candidat** pour OpenFisca — les opérations
centrales (`bincount`, fancy indexing, comparaisons) ne fonctionnent pas
...
**Verdict** : ✅ **Bon candidat** pour le stockage, mais le gain n'est réel
...
class DeltaStorage:
    def store_delta(self, period, array):
        changed = array != self._ref_array    # comparaison O(N)
        self._deltas[period] = {
            'indices': numpy.where(changed)[0],
...
comparaison (~37s) est disproportionné. Ne pas implémenter explicitement
...
**Zstandard (zstd)** est un algorithme de compression créé par Yann Collet
...
L'algorithme parcourt les données avec une fenêtre glissante et remplace
...
sans aucune comparaison explicite, sans gestion de keyframes, sans
...
| **Delta explicite** | ~10-200× | 3-5 jours | Comparaison ~37s, keyframes | 🔴 **Ne pas faire** |
| **Sparse explicite** | ~2-4× | 3-5 jours | Conversion à chaque calcul | 🔴 zstd le fait mieux |
| **Retyping int32→int8** | ~3% global | 1-2 jours | Upcast dans les formules | 🟡 Micro-optimisation |
...
- Le tout sans aucune comparaison explicite, sans gestion de keyframes,
