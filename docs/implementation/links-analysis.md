# Analyse du système de liens de LIAM2 et transposition dans OpenFisca

> Document d'analyse pour l'implémentation de liens inter-entités dans OpenFisca-core,
> inspiré du système `Link` / `Many2One` / `One2Many` de LIAM2.
>
> **Date d'analyse** : 2026-02-22
> **Projets analysés** :
> - LIAM2 : `/home/benjello/projets/liam2`
> - OpenFisca-core : `/home/benjello/projets/openfisca-core`

---

## Table des matières

1. [Le système de liens de LIAM2](#1-le-système-de-liens-de-liam2)
2. [Le système de liens existant dans OpenFisca](#2-le-système-de-liens-existant-dans-openfisca)
3. [Analyse comparative et écarts](#3-analyse-comparative-et-écarts)
4. [Ce que les liens de LIAM2 apporteraient à OpenFisca](#4-ce-que-les-liens-de-liam2-apporteraient-à-openfisca)
5. [Plan d'implémentation](#5-plan-dimplémentation)

---

## 1. Le système de liens de LIAM2

### 1.1 Vue d'ensemble

Dans LIAM2, les **liens** (links) sont des relations nommées entre entités, déclarées dans
...
Les liens relient toujours deux entités via un **champ de liaison** (`link_field`) :
un champ entier contenant l'`id` de l'entité cible.

### 1.2 Déclaration des liens (YAML)
...
### 1.3 Architecture des classes de liens

**Fichier** : `/liam2/liam2/links.py` — 410 lignes

```
Link (base, L19-58)
├── _name: str                    # nom du lien (ex: "household")
...
│   └── __getattr__ = get         # syntaxe point : person.household.rent
│
...
#### Expressions de liens :

```
LinkExpression (L121-154, base abstraite)
├── link: Link                    # le lien utilisé
...
def compute(self, context, link, target_expr, missing_value=None):
    ...
    # 6. Gérer les valeurs manquantes (lien cassé, id = -1)
...
### 1.6 Chaînage de liens

**Fichier** : `links.py:171-201`

LIAM2 permet de chaîner les liens : `person.mother.household.rent`
...
2. Résoudre `household.region` via le lien `household` → résultat par personne
3. Résoudre `mother.X` via le lien `mother` → résultat par personne
4. Résoudre `partner.X` via le lien `partner` → résultat par personne

### 1.7 Liens intra-entité (même entité)

LIAM2 supporte les liens entre individus de **la même entité** :
```yaml
person:
    links:
        mother: {type: many2one, target: person, field: mother_id}
...
- `children.sum(age)` → la somme des âges des enfants
- `mother.dead` → est-ce que la mère est décédée

### 1.8 Le lien `other` (PrefixingLink)

**Fichier** : `links.py:96-118`

LIAM2 fournit un lien spécial `other` utilisé dans le contexte du matching
...
### 1.9 Fonctions temporelles et liens

**Fichier** : `/liam2/liam2/tfunc.py` — 205 lignes

Les liens sont aussi utilisés dans les fonctions temporelles (`lag`, `duration`, `tavg`, `tsum`).
Le mécanisme est le même : `id_to_rownum` permet de mapper les individus entre périodes.

**`fill_missing_values()`** (L15-36) — Le pattern fondamental :
```python
@staticmethod
...
### 1.10 Gestion des liens cassés

Quand un individu référencé est supprimé (décès), le lien pointe vers un `id` dont le
...
Et dans le modèle, l'utilisateur peut explicitement casser un lien :
```yaml
# Après un décès, briser le lien vers la mère
- mother_id: if(mother.dead, UNSET, mother_id)
```

### 1.11 Résolution et attachement des liens

**Fichier** : `entities.py:275-279`

Après le parsing de toutes les entités, les liens sont résolus :
```python
def attach_and_resolve_links(self, entities):
    for link in self.links.values():
        link._attach(self)           # associe le lien à son entité source
        link._resolve_target(entities)  # résout le nom de cible en référence Entity
```

Les liens sont ensuite accessibles comme variables dans le namespace de l'entité :
```python
# entities.py:339
variables.update(self.links)
```

### 1.12 Limites du système de liens de LIAM2

Malgré sa généralité, le système de liens de LIAM2 présente des limitations
...
pour le multi-emploi) nécessitent une entité de jointure. Ce n'est pas un type de lien
primitif.

#### 1.12.2 Pas de sémantique de rôle

Un lien LIAM2 est **générique** : il relie des entités sans qualifier la nature de la
...
```yaml
# LIAM2 : pas de rôle, filtres manuels nécessaires
...
LIAM2 ne vérifie **aucune cohérence** sur les liens :

- **One2One non garanti** : si on déclare un lien `partner` en Many2One, rien n'empêche
...
#### 1.12.4 Pas de métadonnées sur les liens

Un lien est un **simple ID entier**. On ne peut pas attacher d'information à la
relation elle-même (date de début, intensité, type). Il faut des champs séparés :

```yaml
# On ne peut PAS enrichir le lien :
links:
    household: {type: many2one, target: household, field: hh_id,
...
Les liens One2Many fournissent des agrégations (`sum`, `count`, `min`, `max`, `avg`)
...
#### 1.12.7 Pas de liens polymorphes

Un lien pointe toujours vers **une seule entité cible**. On ne peut pas dire « ce
...
#### 1.12.8 Pas de liens temporels croisés

Les liens sont résolus dans le **contexte de la période courante**. On ne peut pas
facilement suivre un lien tel qu'il existait à une période passée :
...
naviguer un lien dans le contexte d'une période passée.
...
| Pas de rôles | Filtres manuels / champs dédiés | **Oui** ✅ (rôles natifs) |
| Pas d'intégrité référentielle | Liens silencieusement cassés | Non |
| Pas de métadonnées sur les liens | Champs séparés nécessaires | Non |
...
| Pas de liens polymorphes | Un lien = une entité cible fixe | Non |
| Pas de liens temporels croisés | Résolution à la période courante | Non |

---

## 2. Le système de liens existant dans OpenFisca

### 2.1 Vue d'ensemble : OpenFisca a bien des liens, mais implicites et structurels

OpenFisca-core possède **déjà** un système de liens entre entités, mais il est
profondément différent de celui de LIAM2. Le choix architectural fondamental d'OpenFisca
est d'avoir **fusionné le concept de lien avec celui d'entité de groupe** (`GroupEntity`).
...
2. members_entity_id          → Lien implicite personne → groupe (dans GroupPopulation)
3. Projectors                 → Navigation syntaxique dans les formules
...
pas via des liens déclaratifs. Chaque `GroupEntity` possède :
...
- Un attribut `max` optionnel sur les rôles pour limiter le nombre d'individus par rôle

```python
# Déclaration d'un GroupEntity (dans un projet pays, ex: openfisca-france)
...
**Comparaison avec LIAM2** :
```yaml
# LIAM2 : lien déclaratif pur, sans notion de rôle
household:
    links:
        persons: {type: one2many, target: person, field: hh_id}
...
**Différence clé** : LIAM2 a un lien générique entre entités. OpenFisca encode la
...
### 2.3 Mécanisme 2 : `members_entity_id` — Le lien implicite personne → groupe

**Fichier** : `populations/group_population.py` (lignes 14-82)

Le lien effectif entre personnes et entités de groupe est incarné par trois tableaux numpy
...
|---|---|---|
| Nature | Variable sur l'entité personne | Attribut de GroupPopulation |
...
OpenFisca n'a **pas besoin** de `id_to_rownum` pour les liens personne↔groupe actuels.
...
Les **projectors** sont le mécanisme qui rend les liens utilisables dans les formules.
...
C'est le seul type de « chaînage de liens » supporté par OpenFisca.
...
def sum(self, array, role=None):          # L94-119 — somme via bincount
...
### 2.7 Résumé : ce qu'OpenFisca fait et ne fait pas avec les liens

#### ✅ Ce qu'OpenFisca fait déjà bien

1. **Lien personne → groupe** : `person.household("rent", period)` — Efficace et naturel
...
6. **Syntaxe dans les formules** : Projectors + `@projectable` — Élégant

#### ❌ Ce qu'OpenFisca ne sait pas faire

1. **Lien personne → personne** : `person.mother("age")` — **Impossible**
2. **Liens nommés personnalisés** : `person.employer("wage")` — **Impossible** sans changer le framework
3. **Chaînage libre** : `person.mother.household("rent")` — **Impossible**
4. **Modification des liens** : changer de ménage — **Impossible** après l'initialisation
...
7. **Lien cassé** : gérer un décès avec `mother_id = -1` — **Impossible**

---

## 3. Analyse comparative et écarts

### 3.1 Le choix architectural fondamental

OpenFisca et LIAM2 ont fait des choix architecturaux **différents mais cohérents** :

**LIAM2** : Les liens sont des **objets de première classe**, déclarés explicitement,
et résolus via un système d'indexation générique (`id_to_rownum`). Tout lien est
un champ entier + une déclaration. C'est **générique** mais sans sémantique :
LIAM2 ne sait pas qui est « parent » ou « enfant ».

**OpenFisca** : Les liens sont **fusionnés avec la structure des entités**. La relation
...
de la sémantique, les projectors rendent la syntaxe naturelle, et les agrégations par rôle
sont directes.

```
           LIAM2                           OpenFisca
           ─────                           ─────────
Philosophie Liens explicites, génériques    Liens implicites, spécialisés
Avantage    Flexibilité totale              Sémantique riche (rôles)
Inconvénient Pas de notion de rôle          Pas de liens intra-entité
...
**C'est le type de lien le mieux supporté** par les deux systèmes (pour le cas inter-entité).
...
- LIAM2 : on accède au lien puis on agrège → `household.persons.sum(age)`
- OpenFisca : on agrège directement sur le GroupPopulation → `household.sum(person("age", period))`

La syntaxe OpenFisca est **moins naturelle** pour les expressions impliquant un filtre :
```python
# LIAM2 : naturel
...
On crée une entité `employment` avec des liens Many2One vers `person` et `employer` :
...
Ce qui permet des calculs comme :
```yaml
# Sur l'entité person :
...
les salaires sont chacun sous le plafond mais dont la somme le dépasse.
...
relations inter-entités au-delà de celles codées dans la structure GroupEntity.

### 3.3 Table de correspondance fonctionnelle détaillée
...
| **Groupe → Personnes (agrégation)** | `household.persons.sum(salary)` (One2Many + Sum) | `household.sum(person("salary", period))` | 🟢 **Équivalent** (syntaxe différente) |
...
| **Chaînage de liens** | `person.mother.household.rent` | Seulement via `containing_entities` (groupe→groupe) | 🟠 **Très limité** |
...
| **Lien cassé (décès)** | `if(mother.dead, UNSET, mother_id)` | N/A (pas de lien dynamique) | 🔴 **Absent** |
...
| **Lien nommé personnalisé** | Oui, déclarés dans YAML | Non, structure fixe (GroupEntity) | 🔴 **Absent** |
...
#### 3.4.1 🔴 Liens intra-entité (personne ↔ personne)

**Le plus important.** LIAM2 permet de créer des liens entre individus de la même entité :
- `mother` : personne → sa mère
- `partner` : personne → son conjoint
- `children` : personne → ses enfants

OpenFisca ne supporte que les liens **personne ↔ groupe** via le système de rôles.
...
#### 3.4.2 🔴 Liens nommés personnalisables

LIAM2 permet de définir **n'importe quel lien** via un simple champ `id` et une déclaration YAML.
OpenFisca a une structure de liens **codée en dur** : personne ↔ ménage/foyer/famille.

On ne peut pas ajouter un lien `employeur` (personne → entreprise) ou `médecin traitant`
...
```yaml
persons.sum(age, age < 18)           # somme des âges des mineurs
...
```python
household.sum(salary, role=Household.CHILD)  # somme des salaires des enfants
```
...
Il est crucial de ne pas perdre les avantages d'OpenFisca dans l'enrichissement :
...
2. **La syntaxe projector** — `person.household("rent", period)` est naturelle et
   les formules sont lisibles
...
4. **La compatibilité** — Des centaines de formules existantes dans les pays
...
| **Sémantique** | Aucune (lien générique) | Riche (rôles, sous-rôles, positions) |
| **Dynamisme** | Les liens peuvent changer (simple assignation de `hh_id`) | Les liens sont fixés à l'initialisation |
| **Chaînage** | Libre et illimité (`a.b.c.d.e`) | Limité à `containing_entities` |

---

## 4. Ce que les liens de LIAM2 apporteraient à OpenFisca

### 4.1 Cas d'utilisation concrets

#### Liens familiaux (intra-entité)
```python
...
#### Liens inter-entités supplémentaires
```python
# Personne → Entreprise
...
```python
# Somme des revenus des membres féminins du ménage de plus de 18 ans
...
### 4.2 Avantages par rapport au système actuel

1. **Expressivité** : Les formules deviennent plus naturelles et lisibles
2. **Généralité** : Plus besoin de bidouiller avec des variables intermédiaires
3. **Extensibilité** : Les pays peuvent définir leurs propres liens
4. **Dynamisme** : Les liens peuvent évoluer (séparations, déménagements)
5. **Intra-entité** : Accès aux caractéristiques d'individus liés (mère, conjoint)

---

## 5. Plan d'implémentation : un système de liens « LIAM2 amélioré »

### 5.1 Vision architecturale

L'approche recommandée est de concevoir un système de liens **à la LIAM2, augmenté
...
```
┌─────────────────────────────────────────────────────────┐
│              Système de liens « LIAM2 amélioré »        │
│                                                         │
│  ┌────────────────────┐   ┌──────────────────────────┐  │
...
### 5.2 L'insight clé : un rôle = une métadonnée sur un lien Many2One

Dans OpenFisca actuel, quand la personne P appartient au ménage H avec le rôle PARENT :
- C'est un lien **Many2One** : personne → ménage (via `members_entity_id`)
...
dans le système de liens « LIAM2 amélioré » :

| # | Fonctionnalité OpenFisca actuelle | Expression avec liens enrichis | Mécanisme |
|---|---|---|---|
...
|---|---|---|---|
| A | Lien intra-entité M2O | `person.mother_link.get("age", period)` | ❌ |
| B | Lien intra-entité O2M | `person.children_link.count()` | ❌ |
...
| F | Lien nommé quelconque | `person.employer_link.get("sector", period)` | ❌ |
| G | Lien dynamique (cassable) | `person.set_link_value("mother_id", -1)` | ❌ |

### 5.4 Classes du système de liens enrichi
...
```python
class Link:
    """Lien entre deux entités (ou au sein de la même entité).

    Un lien est défini par :
    ...
    def attach(self, population):
        """Attacher le lien à sa population source."""
        self._source_population = population
```

#### 5.4.2 `Many2OneLink` — Lien N→1 enrichi de rôles
...
#### 5.4.3 `One2ManyLink` — Lien 1→N enrichi de rôles et d'accès ordonnés
...
class One2ManyLink(Link):
    ...
    def sum(self, variable_name, period, role=None, condition=None,
        ...
        """Somme. Équivalent OpenFisca : GroupPopulation.sum(array, role)"""
...
comme des façades** (thin wrappers) par-dessus le nouveau système de liens :
...
Et `GroupPopulation` peut déléguer ses agrégations au lien sous-jacent :
...
class GroupPopulation(CorePopulation):
    ...
    def sum(self, array, role=None):
        ...
        # APRÈS : délègue au lien implicite
...
### 5.6 Génération automatique de liens depuis les GroupEntity existantes

Au démarrage de la simulation, le système génère automatiquement des liens
à partir de la structure `GroupEntity` + `GroupPopulation` existante :

```python
class Simulation:
    def _generate_links_from_group_entities(self):
        """Générer les liens implicites depuis les GroupEntity existantes.

        Permet à tout code existant de continuer à fonctionner.
        """
...
| **Phase 3** | Génération automatique de liens depuis `GroupEntity` | `simulations/` | 2-3 jours |
| **Phase 4** | Réimplémentation des projectors comme façades | `projectors/` | 2-3 jours |
| **Phase 5** | API pays : déclaration de liens (mother, employer...) | `entities/`, docs | 1-2 jours |
...
1. **Toutes les formules existantes** de tous les pays continuent de
...
5. La déclaration de liens personnalisés fonctionne dans un projet pays de test

### 5.10 Relation avec le PoC « populations dynamiques »

Le système de liens est **complémentaire** au PoC des populations dynamiques :

- **Sans populations dynamiques** : les liens fonctionnent en mode statique, avec des
  variables de liaison fixées à l'initialisation. Les liens intra-entité (mother, children)
  sont possibles mais ne changent pas au cours de la simulation.

- **Avec populations dynamiques** : les liens deviennent pleinement dynamiques. Les
...
**Recommandation** : Implémenter le système de liens **avant** les populations dynamiques.
Les liens en mode statique apportent déjà une valeur significative (liens familiaux,
liens employeur) et constituent le socle sur lequel les populations dynamiques s'appuieront.

### 5.11 Format de déclaration des entités et liens
...
par pays, sans aucune notion de lien explicite :
...
Les liens personne↔groupe sont **implicites** — ils existent du seul fait
qu'il y a un `GroupEntity` avec des rôles. Il n'y a aucune déclaration de lien.

#### 5.11.2 Option A : Déclaration YAML (à la LIAM2)

Cette approche rendrait la structure des entités et liens **déclarative et lisible
d'un coup d'œil** :

```yaml
# entities.yml — OpenFisca-France avec liens explicites
...
entities:
    individu:
        ...
        links:
            # Liens personne → groupe (remplacent l'implicite actuel)
            famille:
                type: many2one
                target: famille
                field: famille_id
            ...
            # Liens intra-entité (NOUVEAUX — impossibles aujourd'hui)
            mere:
                type: many2one
                target: individu
                field: mere_id
...
On voit immédiatement que l'individu a 6 types de liens (3 vers des groupes, 3 intra-entité).

#### 5.11.3 Option B : Déclaration Python enrichie

Cette approche conserve le `build_entity()` existant et ajoute les liens
...
# ── Liens supplémentaires (NOUVEAU) ───────────────────────────────
# Les liens personne ↔ groupe sont auto-générés depuis les GroupEntity.
# On déclare ici les liens SUPPLÉMENTAIRES :
...
# Exemple : lien vers l'employeur (si modélisé)
# Individu.add_link(Many2OneLink('employeur', field='employeur_id', target='entreprise'))

entities = [Individu, Famille, FoyerFiscal, Menage]
```

**Avantage** : Le fichier `entities.py` d'un pays qui n'a **pas encore** de liens
supplémentaires reste **strictement identique** à aujourd'hui. Les 4 lignes `add_link()`
sont le seul ajout.

#### 5.11.4 Option C : Approche mixte (recommandée)

**Python pour les entités, YAML optionnel pour les liens** :

```python
# openfisca_france/entities.py — approche mixte
...
# Liens : chargement optionnel depuis un fichier YAML
load_links('links.yml', entities)    # ← optionnel
```

```yaml
# openfisca_france/links.yml — déclaration des liens supplémentaires
links:
    individu:
...
#### 5.11.5 Comparaison des approches
...
#### 5.11.6 Recommandation : Python d'abord (Option B)

L'approche **Python d'abord** est la plus pragmatique pour le démarrage :

1. **Zéro rupture** — `build_entity()` ne change pas
2. **Les liens personne↔groupe restent implicites** — auto-générés depuis `GroupEntity`
   (cf. §5.6), aucune modification requise dans les pays existants
3. **Les liens supplémentaires sont opt-in** — un pays ajoute `add_link()` uniquement
...
5. **La migration est progressive** — chaque pays peut adopter les liens à son rythme

Concrètement, un pays existant (ex: `openfisca-france`) pourrait ajouter les liens
...
Et les formules pourraient immédiatement utiliser :
```python
class pension_alimentaire(Variable):
    def formula(individu, period):
        ...
        rent_menage = individu.menage("rent", period)    # Many2One inter (existant)
        return ...
```

### 5.12 Extensibilité des opérations sur les liens : l'avantage Python

#### 5.12.1 Le problème de LIAM2 : un vocabulaire fermé

Dans LIAM2, les opérations disponibles sur les liens sont **figées dans le DSL YAML** :
...
**modifier le code source du moteur LIAM2** — ce qui n'est pas à la portée d'un
...
Avec une approche Python, les opérations sur les liens sont des **méthodes Python
ordinaires**. N'importe qui peut en ajouter — dans le framework, dans un package pays,
ou même directement dans une formule :

```python
# ── Niveau 1 : Utilisation directe de numpy dans les formules ──────
...
class EnhancedOne2ManyLink(One2ManyLink):
    """Lien One2Many avec opérations statistiques avancées."""
    ...
    def top_n(self, n, variable_name, period, role=None) -> ndarray:
        """Somme ou valeurs des n plus grandes valeurs par groupe."""
        ...
...
def parts_fiscales(foyer_fiscal, period):
    ...
    Utilise les liens et rôles pour une implémentation lisible.
    """
...
```python
# ── Niveau 4 : Composition libre dans les formules ────────────────
...
class allocation_logement(Variable):
    def formula(menage, period):
        ...
        # Calcul libre, pas de limite du DSL
        rent = menage("rent", period)
        return rent * 0.5 * (1 + ratio_enfants) * (revenu_adultes < 2000)
```

#### 5.12.3 Comparaison : vocabulaire fermé vs ouvert

| Aspect | LIAM2 (YAML DSL) | OpenFisca (Python) |
|---|---|---|
| **Opérations de base** | `sum`, `count`, `avg`, `min`, `max` | ✅ Les mêmes |
| **Ajout d'opérations** | Modifier le moteur LIAM2 | Sous-classer `One2ManyLink` ou écrire du numpy |
...
#### 5.12.4 L'insight : les liens sont des objets, pas des instructions

La différence fondamentale est conceptuelle :

- **LIAM2** : un lien est une **instruction** dans un langage déclaratif.
  Les opérations possibles sont celles que le langage prévoit.

- **OpenFisca avec liens** : un lien est un **objet Python**.
  Les opérations possibles sont celles de Python — donc **illimitées**.

Cela signifie que le framework fournit les primitives (`get`, `sum`, `count`, `nth`,
`get_by_role`...) et que les utilisateurs peuvent :

1. **Combiner** ces primitives librement dans leurs formules
2. **Sous-classer** les types de liens pour ajouter des méthodes
3. **Créer des helpers** spécifiques à leur système pays
4. **Utiliser tout l'écosystème Python** (numpy, scipy, pandas) dans les calculs

C'est un avantage décisif de l'approche Python qui justifie pleinement
la recommandation de ne **pas** remplacer Python par un DSL YAML, mais
de s'inspirer de la **logique** de LIAM2 tout en restant dans Python.

### 5.13 Analyse de performance du système de liens

#### 5.13.1 État actuel : tout est déjà vectorisé

Une analyse exhaustive du code des projectors et de `GroupPopulation` montre
que **toutes les opérations de liens sont déjà vectorisées** en numpy :
...
| **has_role** (filtre) | `members_role == role` | O(N) comparaison | ✅ numpy pur |
...
#### 5.13.2 Les opérations du futur système de liens

Les nouvelles opérations que le système de liens ajouterait sont :
...
#### 5.13.3 Et les liens intra-entité (mère, conjoint) ?

Les nouveaux liens intra-entité (`person.mother`, `person.spouse`) sont
...
Les seuls hotspots du système de liens/projectors sont ceux **déjà identifiés**
...
**Le système de liens n'a aucun problème de performance intrinsèque.**

Toutes les opérations de lien se réduisent à :
- **Fancy indexing** : `array[ids]` → C optimisé dans numpy
- **bincount** : `numpy.bincount(ids, weights)` → C optimisé dans numpy
- **Comparaison vectorielle** : `array == value` → C optimisé dans numpy

Il n'y a **aucun besoin** de Numba, JAX, Cython ou autre accélérateur pour
le système de liens. Les seuls hotspots sont dans les fonctions de groupement
(`members_position`, `reduce`, `get_rank`) qui sont **communes** au système
actuel et au futur système de liens.

L'effort d'optimisation sur les liens se concentre donc sur :

1. **Vectoriser `members_position`** en numpy pur (bénéficie aussi aux
   projectors actuels) — 1 jour, 100× de gain
2. **L'intégrité des liens** : vérifier que les `link_ids` pointent vers
