...
> et extraits de code pertinents des deux projets.
>
...
|---|---|---|---|
| `sum()` | 94-119 | Somme par entité via `bincount` | `members_entity_id`, `self.count` |
...
| `build_from_entities()` | 157-270 | Construit depuis un dict complet (personnes + ménages explicites) |
...
| `set_input(period, array)` | 166-240 | Valide et stocke une entrée | ⚠️ Vérification de taille implicite |
...
Priorité 1 (Phase 0-1) :
  openfisca_core/populations/_core_population.py    ← count/ids → propriétés + attributs dynamiques
