...
| `birth` (date, ETERNITY) | ✅ | ✅ |
| `age` (formule dépendant de birth) | ✅ | ✅ Parfait pour tester que les formulas marchent après mutation |
| `salary` (input, MONTH) | ✅ | ✅ |
| `income_tax` (formule) | ✅ | ✅ |
...
- Le flag est accepté, la frontière advance
...
- Les formulas (y compris les agrégations ménage) donnent les bons résultats après mutation
...
- `age` teste que les formulas fonctionnent après mutation (dépend de `birth`)
...
- Les formulas qui utilisent `period.last_month` ou `ADD` → les arrays
...
### 5. Relation avec le plan des lines

Les populations dynamiques et les lines sont **complémentaires** :
- Les lines (`mother_id`) permettent de modéliser les relations familiales
- Les mutations dynamiques permettent de les faire **évoluer** :
  - Naissance → `add_persons` + `set mother_id`
  - Marriage → `set spouse_id`
  - Divorce → `remove spouse_id`, potentiellement `create new household`

L'idéal est d'implémenter les lines (Phase 1-3 du plan lines) AVANT
...
```
1. Lines Phase 1-3 (classes Link + country-template + branchement)
   → 6-8 jours
2. Évolution Phase 0-1 (flag + evolve + déterminisme)
   → 2-3 jours
3. Évolution Phase 2 (add/remove persons)
   → 3-4 jours
4. Lines Phase 4-5 (auto-génération + façades)
   → 3-5 jours
